...
{
    // 將輸入之16進位數字轉換為各自的10進位(ex: 'A' -> 10)
    // 將數字反向存進整數的array，即string從右至左之element的10進位會由左至右存進array
    // 此轉換可使後續運算較為方便
...
void subtraction(int a[], int b[], int answer[], const int arraysize)
{
    ...
    for(int i = 0; i < arraysize; i++)
    {
        ...
        if(answer[i] < 0)
        {
            ...
            answer[i] += 16;                        // 若計算後為負數，則跟下一位數借16來相加
        }
        else
        {
            borrow = 0;
...
int main()
{
    ...
    while(true)
    {
        ...
        else if (selection == 3)
        {
            ...
            cin >> str1;
            cin >> str2;
            arraysize = str1.length() + str2.length()+1;              // arraysize最大為兩個number(str1與str2)的長度相加再加1
